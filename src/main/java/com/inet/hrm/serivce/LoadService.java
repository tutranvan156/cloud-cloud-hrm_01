...
@Named("loadUser")
@XPortalPageRequest(uri = "user/load", transaction = true, inherit = true, result = WebConstant.ACTION_XSTREAM_JSON_RESULT)
@XPortalDataService(description = "load_user")
public class LoadService extends PluginSubfirmAbstraction {
    ...
    protected WebDataService service(SiteDataModel siteDataModel, SubFirmProfile subFirmProfile, AbstractBaseAction abstractBaseAction, Map<String, Object> map) throws WebOSBOException {
        ...
        return new ObjectWebDataservice<>(userBO.load(name));
